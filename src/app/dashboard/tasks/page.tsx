...
export default async function TasksPage({ searchParams }: { searchParams: Promise<{ priority?: string }> }) {
  ...
  return <div>
    <div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold">Tasks</h1><p className="mt-2 text-slate-500">Concrete actions across the complete project portfolio.</p></div>{projects.length > 0 && <ActionMenu label="Task actions"><CreateTaskDialog projects={projectOptions} triggerVariant="menu" /></ActionMenu>}</div>

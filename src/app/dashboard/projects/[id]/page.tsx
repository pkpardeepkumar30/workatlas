...
export default async function ProjectDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ sort?: string }> }) {
  ...
  return <div>
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <ProjectActionMenu project={project} taskCount={tasks.length} redirectTo="/dashboard/projects" />
    </div>
    ...
    <Card className="mt-6">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <ActionMenu label="Project task actions"><CreateTaskDialog projectId={project.id} projectTitle={project.title} triggerVariant="menu" /></ActionMenu>
        </div>
      </div>
      <CardContent className="space-y-3">
        {tasks.map((task) => <article key={task.id} className="rounded-xl border border-slate-200 p-4">
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3">
            ...
            <TaskActionMenu task={task} projects={projectOptions} />
          </div>
        </article>)}
